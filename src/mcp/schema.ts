...
export const toolSchemas = {
  ...
  set_breakpoint: {
    name: 'set_breakpoint',
    description: 'Switch the active responsive breakpoint. Desktop-first: "base" = LG default (≥1280px), "xl" = MD medium (768–1280px), "md" = SM small (≤768px). After switching, ALL update_frame/update_spacing/update_size calls write to that breakpoint\'s overrides instead of the base frame. Only changed properties are stored (sparse overrides). The canvas width is also adjusted to match. Call set_breakpoint({ breakpoint: "base" }) to return to default editing when done.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        breakpoint: { type: 'string', enum: ['base', 'md', 'xl'], description: 'The breakpoint to activate. "base" = LG default ≥1280px, "xl" = MD medium 768–1280px, "md" = SM small ≤768px.' },
      },
      required: ['breakpoint'],
    },
  },
